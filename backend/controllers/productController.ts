  ...
  if (!existingItem) {
    ...
    try {
      ...
      const onSale: boolean = response.data.offers[0].badges.dealBadge
        ? true
        : false;
      const saleEndDate: Date =
        response.data.offers[0].badges.dealBadge?.expiryDate ?? null;
      const saleType: string | null =
        response.data.offers[0].badges.dealBadge?.type;
      const saleText: string | null =
        response.data.offers[0].badges.dealBadge?.text ?? null;
      ...
      // TODO: Fix this so it gives accurate data
      const saleUnitPrice: number | null = salePrice
        ? salePrice / divisor
        : null;
      ...
      console.log(err);
      console.log(
        "This code is invalid or item is not currently available at this location"
      );
    }
  } else {
...
export const updateProduct = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      // COMPARISON INFO
      const compQty: number =
        response.data.offers[0].comparisonPrices[0].quantity; // WILL THEY ALWAYS HAVE THIS?
      const divisor: number = packageSizeNum / compQty;
      const unitPrice: number = price / divisor;
